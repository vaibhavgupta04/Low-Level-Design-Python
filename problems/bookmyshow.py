...
class Booking:
    ...
    class BookingBuilder:
        ...
        def build(self) -> 'Booking':
            # Validate required fields and provide defaults where sensible
            if self.user is None:
                raise ValueError("Booking must have a user")
            if self.show is None:
                raise ValueError("Booking must have a show")
            if not self.seats:
                raise ValueError("Booking must have at least one seat")
            if self.total_amount is None:
                raise ValueError("Booking must have a total_amount")
            if self.payment is None:
                raise ValueError("Booking must have a payment")

            booking_id = self.id if self.id is not None else f"BKG{uuid.uuid4()}"
            return Booking(booking_id, self.user, self.show, self.seats, self.total_amount, self.payment)
...
class MovieSubject:
    ...
    def notify_observers(self) -> None:
        for observer in self.observers:
            # `self` is a Movie (subclass of MovieSubject) at runtime; cast for type-checkers
            observer.update(cast("Movie", self))
